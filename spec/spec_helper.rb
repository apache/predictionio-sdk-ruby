...
RSpec.configure do |config|
  config.before(:each) do
    ...
    stub_request(:post, 'http://fakeapi.com:8000/events.json')
      .with(body: { appId: 1, event: 'register', entityType: 'user',
                    entityId: 'foobar' })
      .to_return(status: 201, body: JSON.generate(eventId: 'deadbeef00'))
    stub_request(:post, 'http://fakeapi.com:8000/events.json')
      .with(body: { appId: 1, event: '$set', entityType: 'pio_user',
                   entityId: 'foobar' })
      .to_return(status: 201, body: JSON.generate(eventId: 'deadbeef01'))
    stub_request(:post, 'http://fakeapi.com:8000/events.json')
      .with(body: { appId: 1, event: '$unset', entityType: 'pio_user',
                    entityId: 'foobar', properties: { bar: 'baz' } })
      .to_return(status: 201, body: JSON.generate(eventId: 'deadbeef02'))
    stub_request(:post, 'http://fakeapi.com:8000/events.json')
      .with(body: { appId: 1, event: '$set', entityType: 'pio_item',
                    entityId: 'foobar' })
      .to_return(status: 201, body: JSON.generate(eventId: 'deadbeef03'))
    stub_request(:post, 'http://fakeapi.com:8000/events.json')
      .with(body: { appId: 1, event: '$unset', entityType: 'pio_item',
                    entityId: 'foobar', properties: { bar: 'baz' } })
      .to_return(status: 201, body: JSON.generate(eventId: 'deadbeef04'))
    stub_request(:post, 'http://fakeapi.com:8000/events.json')
      .with(body: { appId: 1, event: 'greet', entityType: 'pio_user',
                    entityId: 'foobar', targetEntityType: 'pio_item',
                    targetEntityId: 'barbaz', properties: { dead: 'beef' } })
